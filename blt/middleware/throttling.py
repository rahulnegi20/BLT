...
class ThrottlingMiddleware:
    ...
    def __call__(self, request):
        ...
        if self.is_throttled(request):
            logger.warning(
                "Request throttled: %s %s from IP: %s (limit exceeded)",
                method, path, ip
            )
    ...
    def is_throttled(self, request):
        ...
        logger.debug(
            "Throttle check for %s %s from IP: %s - Current: %d/%d",
            method, request.path, ip, current_count, limit
        )
        
        if current_count >= limit:
            logger.warning(
                "Rate limit exceeded for %s requests from IP: %s - Count: %d/%d",
                method, ip, current_count, limit
            )
            return True
        ...
        logger.debug(
            "Incremented throttle counter for %s from IP: %s - New count: %d/%d",
            method, ip, new_count, limit
        )
